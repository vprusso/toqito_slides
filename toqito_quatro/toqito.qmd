---
title: "|toqito⟩"
author:
    name: vincent russo
    url: https://vprusso.github.io/
    email: vincentrusso1@gmail.com
date: "2025-05-29"
footer: "{{< fa brands github >}} [https://github.com/vprusso/toqito](https://github.com/vprusso/toqito)"
format:
    revealjs:
        slide-number: c/t
        preview-links: true
        css: styles.css
        math: katex
---

# What's in the name?

::: {.fragment .fade-in-then-semi-out}
|`toqito`⟩
`T`heory `o`f `q`uantum `i`nformation `t`oolkit
:::

::: {.fragment .fade-in-then-semi-out}
Open-source Python library for studying objects in quantum information.
:::

::: {.fragment}
Numerical tools to study problems about entanglement theory, nonlocal games, matrix analysis, and other aspects of
quantum information often associated with computer science.
:::

# Motivation

::: {.fragment .fade-in-then-semi-out}
Tool to rapidly iterate on prototyping research ideas and approaches to problems in quantum information theory. 
:::


# Perks

- 💻 Open-source
- ⚛️ Used for quantum information research
- 📄 Cited in peer-reviewed articles
- 🧪 Thorough and rigorous testing
- 🚀 Optimized for speed

# Externals

- 🥇 1st place KaiCode (software quality out of 400+ projects)
- 👨‍💻 Participating project in unitaryHack (2022-2025)
- 🔆 Google Summer of Code (2025)
- 💵 Unitary Foundation microgrant recipient (2021, 2022)

# Installing `|toqito⟩`

::: {.fragment .fade-in-then-semi-out}
**Install**
```sh
$ pip install toqito
```
:::

::: {.fragment}
**Run!**
```python
>>> import toqito
>>> toqito.states.bell(0)
array([[0.70710678],
       [0.        ],
       [0.        ],
       [0.70710678]])
```
:::

# Applications

- ❓ Quantum state distinguishability and exclusion
- 🎮 Bound values of nonlocal games
- ⚛️ Determine properties of quantum states and channels
- 🎲 Generate random quantum states, channels, etc. 

# Quantum states

## Density operators

::: {.fragment .fade-in-then-semi-out}
Quantum states can be represented as *density operators*.
:::

::: {.fragment .fade-in-then-semi-out}
$$
\rho \succeq 0 \quad \text{and} \quad \text{Tr}(\rho) = 1
$$
:::

::: {.fragment .fade-in-then-semi-out}
```python
import numpy as np
from toqito.rand import random_density_matrix
from toqito.matrix_props import is_positive_semidefinite

rho = random_density_matrix(4)

assert np.isclose(np.trace(rho), 1) and is_positive_semidefinite(rho)
```
:::

## Entanglement and separability

::: {.fragment .r-stack .fade-in-then-semi-out}
A quantum state is *entangled* if it is *not separable*. A separable state can be written as 
$$
\rho = \sum_{i} p_i \rho_i^{A} \otimes \rho_j^{B}
$$
:::

::: {.fragment}
```python
from toqito.states import bell
from toqito.state_props import is_separable

is_separable(bell(0) @ bell(0).conj().T)
```
:::

::: {.callout-warning title="Computability" .fragment .fade-in-then-out}
Separability testing is NP-hard.
:::


## Quantum state exclusion

![](images/qse_figure.svg){width=20% fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out}
- Alice prepares $\rho_i \in \{\rho_1, \ldots, \rho_k \}$.
- Alice sends $\rho_i$ to Bob.
- Bob wins if he correctly *excludes* an index $j \neq i$.
:::

## Quantum state exclusion

$$
\min \sum_{i=1}^k p_i \text{Tr}(M_i \rho_i)
$$

::: {.fragment}
```python
from toqito.states import bell
from toqito.state_opt import state_exclusion

states = [bell(0), bell(1), bell(2), bell(3)]
state_exclusion(states)
```
:::

## An SDP for state exclusion


$$
\begin{equation}
    \begin{aligned}
        \text{minimize:} \quad & \sum_{i=1}^{n} p_i\text{Tr}(M_i \rho_i)  \\
        \text{subject to:} \quad & \sum_{i=1}^{n} M_i = \mathbb{I}, \\
                                    & M_i \succeq 0, \quad \forall \ 1 \leq i \leq n.
    \end{aligned}
\end{equation}
$$


## Antidistinguishability {.smaller}
$$
\sum_{i=1}^k \text{Tr}(M_i \rho_i) = 0
$$

::: {.fragment}
- Perfect conclusive quantum state exclusion.
- Connections to quantum foundations.
- Related to distinguishability, but less well-studied.
- Applications to other areas of quantum information.
- Connections to SIC-POVMs, frame theory, spherical codes, etc.
:::

::: {.fragment}
```python
from toqito.states import bell
from toqito.state_props import is_antidistinguishable

states = [bell(0), bell(1), bell(2), bell(3)]
is_antidistinguishable(states)
```
:::

## Conjecture

Let $S = \{|\psi_1\rangle, \ldots, |\psi_n\rangle \} \subset \mathbb{C}^n$ be a set of $n$ pure quantum states each of
dimension $n$. Then $S$ is antidistinguishable when

$$
|\langle \psi_i | \psi_j \rangle | \leq \frac{n-2}{n-1}
$$

for all $i \neq j$.

::: {.fragment .r-stack .fade-in-then-semi-out}
- Would imply existence of quantum communication task with exponential advantage over classical.
- Unfortunately, conjecture is `false`.
:::

## Finding a counterexample

::: {.fragment .r-stack .fade-in-then-semi-out}
- Randomly generate set of quantum states of fixed dimension.
- Check if bound is satisfied and `not` antidistinguishable.
:::

::: {.fragment}
```python
import numpy as np
from toqito.rand import random_states
from toqito.state_props import is_antidistinguishable

S = random_states(n=4, d=4)
threshold = (len(S) - 2) / (len(S) - 1)

for i in range(len(S)):
    for j in range(i + 1, len(S)):
        ip_val = np.abs(np.trace(S[i].conj().T @ S[j]))
        if ip_val > threshold and not is_antidistinguishable(S):
            print("Violation found!")
```
:::

## Finding a counterexample

::: {.fragment .r-stack .fade-in-then-semi-out}
![](images/logo.svg){width=300 fig-align="center"}
:::

::: {.fragment .r-stack .fade-in-then-semi-out}
![](images/rpi.png){width=300 fig-align="center"}
:::

::: {.fragment .r-stack .fade-in-then-semi-out}
![](images/adist_paper.png){width=80% fig-align="center"}
:::

## Picking up the pieces {.smaller}

::: {.fragment}
**Sufficient condition for antidistinguishability** [Johnston, R. Sikora]:

Let $S = \{|\psi_1\rangle, \ldots, |\psi_n \rangle \}$ be a collection of $n$ states. If
$$
    \begin{equation}
        |\langle\psi_i|\psi_j\rangle| \leq \frac{1}{\sqrt{2}}\sqrt{\frac{n-2}{n-1}}
    \end{equation}
$$ for all $i \neq j$ then $S$ is antidistinguishable.
:::

::: {.fragment}
**Necessary condition for antidistinguishability** [Johnston, R. Sikora]:

Let $n \geq 2$ and let $S = \{|\psi_1\rangle, \ldots, |\psi_n\rangle\}$ be a collection of $n$ states. If
$$
    \begin{equation}
        |\langle \psi_i | \psi_j \rangle| > \frac{n-2}{n-1}
    \end{equation}
$$
for all $i \neq j$ then $S$ is not antidistinguishable
:::



## Quantum state distinguishability

![](images/qsd_figure.svg){width=20% fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out}
- Alice prepares $\rho_i \in \{\rho_1, \ldots, \rho_k \}$.
- Alice sends $\rho_i$ to Bob.
- Bob wins if he correctly identifies the index $i$.
:::


## Quantum state distinguishability

$$
\max \sum_{i=1}^k p_i \text{Tr}(M_i \rho_i)
$$

::: {.fragment}
```python
from toqito.states import bell
from toqito.state_opt import state_distinguishability

states = [bell(0), bell(1), bell(2), bell(3)]
state_distinguishability(states)
```
:::

## Optimizing over measurements

![](images/measurement_inclusions.svg){height=0.5, fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out}
- Global: Optimize over *all* valid POVMs (SDP)
- PPT: Partial transpose is positive semidefinite (SDP)
- SEP: Separable operators (SDP(s))
- LOCC: Complex mathematical characterization
:::

## PPT distinguishability

::: {.fragment}
$$
\max_{M_i \in \text{PPT}} \sum_{i=1}^k p_i \text{Tr}(M_i \rho_i)
$$
:::

::: {.fragment}
$$
S = \{ 
    \Phi^+ \otimes \Phi^+,
    \Phi^- \otimes \Psi^-,
    \Psi^+ \otimes \Psi^-,
    \Psi^- \otimes \Psi^-
\}
$$
:::

::: {.fragment}
```python
from toqito.states import bell
from toqito.state_opt import ppt_distinguishability

states = [
    np.kron(bell(0), bell(0)), 
    np.kron(bell(1), bell(3)),
    np.kron(bell(2), bell(3)),
    np.kron(bell(3), bell(3)),
]
ppt_distinguishability(states, dimensions=[2, 2, 2, 2], subsystems=[0, 2])
```
:::

# 

**Resource-assisted state discrimination** 

*Question*: How much does entanglement help for distinguishing quantum states?

::: {.fragment}
$$
\text{Resource state: } |\tau\rangle = \sqrt{\frac{1 + \epsilon}{2}}|00 \rangle + \sqrt{\frac{1 - \epsilon}{2}}|11\rangle
$$

$\epsilon \in [0,1]$ ($\epsilon=0$: maximally entangled, $\epsilon=1$: product)
:::

::: {.fragment}
$$
\{ |\psi_1\rangle \otimes |\tau\rangle, \ldots, |\psi_n\rangle \otimes |\tau\rangle \}
$$
:::

#  {.smaller}

**Resource-assisted state discrimination**

::: {.fragment}
![](images/bell_ppt.png){fig-align="center"}
:::

::: {.fragment}
```python
import numpy as np
from toqito.states import bell
from toqito.state_opt import ppt_distinguishability

eps = 0.5
e0, e1 = np.array([[[1], [0]]), np.array([[0], [1]])
tau = np.sqrt((1+eps)/2) * e0 + np.sqrt((1-eps) / 2) * e1
states = [np.kron(bell(0), tau), np.kron(bell(1), tau), np.kron(bell(2), tau), np.kron(bell(3), tau)]

ppt_distinguishability(states, dimensions=[2, 2, 2, 2], subsystems=[0, 2])

1 / 2 * (1 + np.sqrt(1 - eps**2))
```
:::

<!-- ## Quantum sequence discrimination

-  -->

# Nonlocal games

## The players

![](images/alice_and_bob.svg){width=80% fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out}
The players: Alice and Bob
:::

::: {.fragment .r-stack .fade-in-then-semi-out}
Play cooperatively. Cannot communicate once game begins.
:::

## The referee

![](images/referee.svg){fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out} 
The referee
:::

::: {.fragment .r-stack .fade-in-then-semi-out}
Gives questions. Takes answers. Decides fate.
:::

## Nonlocal game

![](images/nonlocal_game.svg){width=40% fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out}
- Referee gives $x$ to Alice and $y$ to Bob.
- Alice responds with $a$ and Bob responds with $b$.
- Decides fate based on questions $(x,y)$ and answers $(a,b)$.
:::

## Strategies and values

::: {.fragment .r-stack .fade-in-then-semi-out}
- Value: maximum probability with which Alice and Bob win.
- Players can use different *strategies* (based on their resources).
:::

::: {.fragment .r-stack .fade-in-then-semi-out}
$$
\omega(G) \leq \omega^*(G) \leq \omega_{\text{c}}(G) \leq \omega_{\text{ns}}(G)
$$ 
:::


## XOR games

::: {.fragment .r-stack .fade-in-then-semi-out}
Games where the winning condition uses the $\oplus$ operation.
:::

::: {.callout-warning title="Terminology" .fragment .fade-in-then-out}
Values of XOR games can be solved directly. Not true in general for nonlocal games
:::

## CHSH game

::: {.fragment .r-stack .fade-in-then-semi-out}
$$
a \oplus b = x \land y
$$
:::


::: {.fragment}
```python
# Calculate the classical and quantum value of the CHSH game.
import numpy as np
from toqito.nonlocal_games.xor_game import XORGame

# The probability matrix.
prob_mat = np.array([[1/4, 1/4], [1/4, 1/4]])

# The predicate matrix.
pred_mat = np.array([[0, 0], [0, 1]])

# Define CHSH game from matrices.
chsh = XORGame(prob_mat, pred_mat)

chsh.classical_value() # 0.75
chsh.quantum_value() # 0.8535533
```
:::

## Bounding the quantum value

*NPA hierarchy*: Hierarchy of SDPs that converges to $\omega_{\text{c}}(G)$.

::: {.fragment .r-stack .fade-in-then-semi-out}
![](images/qc_hierarchy_levels.svg){height=300 fig-align="center"}
:::



## The FFL game

::: {.fragment .fade-in-then-semi-out}
The FFL (Fortnow, Feige, Lovasz) game:
$$
a \lor x = b \lor y
$$
:::

::: {.fragment .fade-in-then-semi-out}
$$
\begin{equation}
    \pi(0,0) = \pi(0,1) = \pi(1,0) = \frac{1}{3}, \ \pi(1,1) = 0 \\
\end{equation}
$$
:::

::: {.fragment .fade-in-then-semi-out}
$$
\begin{equation}
    \begin{aligned}
        (x,y) &\in \{0,1\} \times \{0,1\} \\
        (a,b) &\in \{0,1\} \times \{0,1\}
    \end{aligned}
\end{equation}
$$
:::


## The FFL game {.smaller}

::: {.fragment .fade-in-then-semi-out}
```python
import numpy as np
from toqito.nonlocal_games.nonlocal_game import NonlocalGame

num_alice_in, num_alice_out = 2, 2
num_bob_in, num_bob_out = 2, 2

prob_mat = np.array([[1/3, 1/3], [1/3, 0]])

pred_mat = np.zeros((num_alice_out, num_bob_out, num_alice_in, num_bob_in))
for a_alice in range(num_alice_out):
    for b_bob in range(num_bob_out):
        for x_alice in range(num_alice_in):
            for y_bob in range(num_bob_in):
                if (a_alice or x_alice) != (b_bob or y_bob):
                    pred_mat[a_alice, b_bob, x_alice, y_bob] = 1
ffl = NonlocalGame(prob_mat, pred_mat)

ffl = NonlocalGame(prob_mat, pred_mat)
print(f"Classical value: {ffl.classical_value()}")
print(f"Quantum value (lower bound): {ffl.quantum_value_lower_bound()}")
print(f"Quantum value (upper bound): {ffl.commuting_measurement_value_upper_bound()}")
print(f"Non-signaling value: {ffl.nonsignaling_value()}")
```
:::


## Extended nonlocal games (ENLG)

![](images/extended_nonlocal_game.svg){width=40% fig-align="center"}

::: {.fragment .r-stack .fade-in-then-semi-out}
- Referee gives $x$ to Alice and $y$ to Bob.
- Alice responds with $a$ and Bob responds with $b$.
- Decides fate based on questions $(x,y)$ and answers $(a,b)$.
:::

## ENLG applications

- Generalization of nonlocal game
- Device-independent cryptographic scenarios
- Quantum steering (intermediate between Bell nonlocality and quantum entanglement)

## ENLG with quantum advantage {.smaller}

```python
from toqito.nonlocal_games.extended_nonlocal_game (
    import ExtendedNonlocalGame
)
from toqito.states import mutually_unbiased_basis
import numpy as np

num_in, num_out = 4, 3
prob_mat = np.eye(num_in) / num_in
mubs = mutually_unbiased_basis(num_out)

pred_mat = np.zeros(
    (num_out, num_out, num_out, num_out, num_in, num_in), dtype=complex
)
for i, basis in enumerate(mubs):
    for j, psi in enumerate(basis):
        pred_mat[..., j, j, i, i] = psi @ psi.conj().T

g_mub = ExtendedNonlocalGame(prob_mat, pred_mat)
print(f"Unentangled value: {g_mub.unentangled_value()}")
print(f"Quantum value: {g_mub.quantum_value_lower_bound()}")
```

## Open problems

::: {.fragment .r-stack .fade-in-then-semi-out}
![](images/open_problems.png){height=300 fig-align="center"}
[https://github.com/vprusso/toqito/wiki](https://github.com/vprusso/toqito/wiki)
:::


# Got an idea? {.smaller}

::: {.columns}

::: {.column}
**Apply for a microgrant!**
![](images/microgrants.png)
:::

::: {.column}
**Approach:**

- $4k no strings attached
- Aimed at explorers in quantum
- Generally open-source, but also community projects
- Usually 3-6 months duration
- Short online applications

::: {.fragment}
::: {.callout-tip appearance="simple" style="font-size: 1.3em;"}
Apply @ <code>[**unitary.foundation/grants**]{style="color:green;"}</code>
:::
:::
:::

:::

# Get involved!

:::: {.columns}

::: {.column width="50%"}

::: {.callout-note title="We want you!" style="text-align:left;!important"}

1. Contribute a new feature
2. Use for research
3. Add documentation
4. Promote `|toqito⟩`
:::

:::

::: {.column width="49%"}
repo: 

[`vprusso/toqito`](https://github.com/vprusso/toqito)
:::

::::

#

:::: columns

::: {.column width="45%"}
<img src="images/toqito_qrcode.png" alt="metriq-gym repo QR Code" style="width: 90%; vertical-align: middle;">
:::

::: {.column width="55%" style="text-align:right; vertical-align: middle;"}
[**Thank you!**]{.r-fit-text}
:::

::::
