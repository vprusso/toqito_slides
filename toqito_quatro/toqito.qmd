...
::: {.fragment .fade-in-then-semi-out}
The FFL (Fortnow, Feige, Lovasz) game is a nonlocal game specified as follows.
$$
\begin{equation}
    \begin{aligned}
        \pi(0,0) = \frac{1}{3}
...
# FFL game input probability matrix.
prob_mat = np.array([[1/3, 1/3], [1/3, 0]])

# Predicate matrix: win if (a ⊕ x) ≠ (b ⊕ y)
pred_mat = np.zeros((num_out, num_out, num_in, num_in))
...
# Define and evaluate the game.
ffl = NonlocalGame(prob_mat, pred_mat)
print(f"Classical value: {ffl.classical_value():.2f}")
print(f"Quantum value (lower bound): {ffl.quantum_value_lower_bound():.2f}")
```
:::
